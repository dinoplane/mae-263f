...
def computeTangent(q):
  ne = int((len(q)+1)/4 - 1) # number of edges = number of nodes - 1
  tangent = np.zeros((ne, 3))
  for c in range(ne):
    dx = q[4*c+4:4*c+7] - q[4*c:4*c+3] # edge vector (skip angle)
...
def discrete_elastic_rods(qGuess, q0, u, a1, a2,
           ...
           Fg):
    """
    Function to compute the discrete elastic rods.
    returns:
        q: Position vector (DOF vector) at the next time step
        u: Velocity vector at the next time step
        a1: Reference frame at the next time step
        a2: Reference frame at the next time step # Not really needed
    """
...
def ch6asg1():
    ...
    x0 = np.array([0,       0,     0])
    x1 = np.array([0.5,     0,     0])
    x2 = np.array([0.75,    0.25,  0])
    x3 = np.array([0.75,    0.5,   0.25])
    x = np.array([x0, x1, x2, x3])

    # material directors mk1
    # these are needed because nodal coordinates alone cannot show whether or not the rod is twisted
    m01 = np.array([0, 0, 1])
    m11 = np.array([0, 0, 1])
    m21 = np.array([0, -1/np.sqrt(2), 1/np.sqrt(2)])
    mn1 = np.array([m01, m11, m21])
    ...
    # integrated twist in rod
    twist = np.zeros(2)

    for k in range(1, 3):
        mk_minus_1 = mn1[k-1]
        Pk1 = parallel_transport(mk_minus_1, t[k-1], t[k])
        twist[k-1] = signedAngle(Pk1, mn1[k], t[k])

    print(f"Ch6 Asg1: Twist: {twist}")
...
# rod with nodes at xk
    ...
    # first director of e1
    u0 = np.array([0, 0, 1])
    v0 = np.cross(t0, u0)
    u = np.array([u0])
    v = np.array([v0])
    for k in range(1, 3):
        u = np.vstack((u, parallel_transport(u[k-1], t[k-1], t[k])))
        v = np.vstack((v, parallel_transport(v[k-1], t[k-1], t[k])))
    # print(t)
    # print(u)
    # print(v)

    twist_angles = np.zeros(3)
    for k in range(3):
        twist_angles[k] = signedAngle(u[k], mn1[k], t[k])

    # print(twist_angles)

    twist = np.zeros(2)
    for k in range(1, 3):
        twist[k-1] = twist_angles[k] - twist_angles[k-1] 
    print(f"Ch6 Asg 2: Twist: {twist}")

def ch6asg3():
    nv = 5
    q0 = np.array([ 2.000e-02,  0.000e+00,  0.000e+00,  0.000e+00, 
           6.306e-03,  1.898e-02,  0.000e+00,  0.000e+00, 
          -1.602e-02,  1.197e-02,  0.000e+00,  0.000e+00, 
          -1.641e-02, -1.143e-02,  0.000e+00,  0.000e+00,  
           5.673e-03, -1.918e-02,  0.000e+00  ])

    q1 = np.array([ 2.000e-02,  0.000e+00,  0.000e+00,  0.000e+00, 
           6.306e-03,  1.898e-02,  0.000e+00, -2.006e-01, 
          -1.463e-02,  1.281e-02, -8.462e-03,  2.191e-01, 
          -1.441e-02, -8.443e-03, -1.827e-02,  4.726e-01, 
           7.321e-03, -1.712e-02, -1.796e-02])

    q2 = np.array([ 2.000e-02,  0.000e+00,  0.000e+00,  0.000e+00, 
           6.306e-03,  1.898e-02,  0.000e+00, -3.908e-01, 
          -1.119e-02,  1.474e-02, -1.497e-02,  3.572e-01, 
          -9.813e-03,  3.396e-04, -3.337e-02,  9.616e-01, 
           1.117e-02, -9.421e-03, -3.688e-02 ])

    u0 = np.array([-8.110e-01, -5.851e-01, -0.000e+00])
    refTwist = np.zeros(nv)

    print("Ch6 Asg 3: t = 0 ---------------------------------------------------------------- ")
    a1, a2 = computeSpaceParallel(u0, q0)
    print(f"Ch6 Asg 3: a1: {a1}")
    material_frame = computeMaterialFrame(a1, a2, q0[3::4])
    t0 = computeTangent(q0)
    refTwist = getRefTwist(a1, t0, refTwist)
    print(f"Ch6 Asg 3: refTwist: {refTwist}")

    twist = np.zeros(5)
    for k in range(1, 4):
        twist[k] = q0[4*k + 3] - q0[4*(k-1)+ 3] + refTwist[k]
    print(f"Integrated Discrete Twist: {twist}")


    print("Ch6 Asg 3: t = 0.05 ---------------------------------------------------------------- ")
    a1, a2 = computeTimeParallel(a1, q0, q1)
    print(f"Ch6 Asg 3: a1: {a1}")
    material_frame = computeMaterialFrame(a1, a2, q1[3::4])
    t1 = computeTangent(q1)
    refTwist = getRefTwist(a1, t1, refTwist)
    print(f"Ch6 Asg 3: refTwist: {refTwist}")

    twist = np.zeros(5)
    for k in range(1, 4):
        twist[k] = q1[4*k + 3] - q1[4*(k-1)+ 3] + refTwist[k]
    print(f"Integrated Discrete Twist: {twist}")


    print("Ch6 Asg 3: t = 0.10 ---------------------------------------------------------------- ")
    a1, a2 = computeTimeParallel(a1, q1, q2)
    print(f"Ch6 Asg 3: a1: {a1}")
    material_frame = computeMaterialFrame(a1, a2, q2[3::4])
    t2 = computeTangent(q2)
    refTwist = getRefTwist(a1, t2, refTwist)
    print(f"Ch6 Asg 3: refTwist: {refTwist}")

    twist = np.zeros(5)
    for k in range(1, 4):
        twist[k] = q2[4*k + 3] - q2[4*(k-1)+ 3] + refTwist[k]
    print(f"Integrated Discrete Twist: {twist}")
...
def ch7asg1():
    ...
    for timeStep in range(Nsteps):
        ...
        qGuess = q0.copy() # This should be fixed
        q, u, a1, a2 = discrete_elastic_rods( qGuess, # Guess solution
                            q0, u, a1, a2,
                            freeIndex, # Boundary conditions
                            dt, tol, # time stepping parameters
                            refTwist, # We need a guess refTwist to compute the new refTwist
